...
/**
 * BoardGameTracker is a child of AlexaSkill.
 * To read more about inheritance in JavaScript, see the link below.
 *
 * @see https://developer.mozilla.org/en-US/docs/Web/JavaScript/Introduction_to_Object-Oriented_JavaScript#Inheritance
 */
var BoardGameTracker = function () {
    AlexaSkill.call(this, APP_ID);
};

// Extend AlexaSkill
BoardGameTracker.prototype = Object.create(AlexaSkill.prototype);
BoardGameTracker.prototype.constructor = BoardGameTracker;

// ----------------------- Override AlexaSkill request and intent handlers -----------------------

BoardGameTracker.prototype.eventHandlers.onSessionStarted = function (sessionStartedRequest, session) {
    console.log("onSessionStarted requestId: " + sessionStartedRequest.requestId
        + ", sessionId: " + session.sessionId);
    // any initialization logic goes here
};

BoardGameTracker.prototype.eventHandlers.onLaunch = function (launchRequest, session, response) {
    console.log("onLaunch requestId: " + launchRequest.requestId + ", sessionId: " + session.sessionId);
    handleWelcomeRequest(response);
};

BoardGameTracker.prototype.eventHandlers.onSessionEnded = function (sessionEndedRequest, session) {
 ...
 */
BoardGameTracker.prototype.intentHandlers = {
    "OneshotBoardGameTrackerIntent": function (intent, session, response) {
        handleOneshotBoardGameTrackerRequest(intent, session, response);
    },

    "DialogBoardGameTrackerIntent": function (intent, session, response) {
       handleDialogBoardGameTrackerRequest(intent, session, response);
    },
...
// -------------------------- BoardGameTracker Domain Specific Business Logic --------------------------
...
function handleWelcomeRequest(response) {
    var whatGame = "What game would you like to log a play for?",
        speechOutput = {
            speech: "Welcome to Board Game Tracker. "
                + whatGame,
            type: AlexaSkill.speechOutputType.PLAIN_TEXT
        },
        repromptOutput = {
            speech: "You can simply respond with the name of a game like "
                + "Dominion. "
                + whatGame,
            type: AlexaSkill.speechOutputType.PLAIN_TEXT
        };
...
function handleHelpRequest(response) {
    var repromptText = "What game would you like to log a play for?";
    var speechOutput = "You can simply respond with the name of a game like "
        + "Dominion.  "
        + repromptText;
 ...
 */
function handleOneshotBoardGameTrackerRequest(intent, session, response) {
...
function getFinalTideResponse(cityStation, date, response) {
    ...
    makeTideRequest(cityStation.station, date, function tideResponseCallback(err, highTideResponse) {
        ...
        response.tellWithCard(speechOutput, "BoardGameTracker", speechOutput)
    });
}
...
exports.handler = function (event, context) {
    var BoardGameTracker = new BoardGameTracker();
    BoardGameTracker.execute(event, context);
};
